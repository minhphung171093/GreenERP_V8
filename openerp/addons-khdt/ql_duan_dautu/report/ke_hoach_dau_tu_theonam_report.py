...
class Parser(report_sxw.rml_parse):
    ...
    def khoitao_danhmuc(self, danhmuc_cha=[]):
        ...
        from_date = wizard_data['date_from']
        to_date = wizard_data['date_to']
        nam_obj = self.pool.get('khdt.nam')
        nam_ids = wizard_data['nam_ids']
        nams = [str(n.name) for n in nam_obj.browse(self.cr, self.uid, nam_ids)]
#         nams = str(nams).replace('[','(')
#         nams = str(nams).replace(']',')')
...
#         danhmucs = danhmuc_obj.browse(self.cr, self.uid, self.danhmuc_arr)
        ...
        for giaidoan in giaidoan_obj.search(self.cr, self.uid, []):
            sql = '''
                ...
                WHERE giai_doan_id = %(giaidoan)s 
                   and danhmuc_id = %(danhmuc)s
                   and id in (select duan_dautu_id from duan_dautu_thuchien where nam in %(from_date)s)
            ''' % {'giaidoan': giaidoan,
                 'danhmuc': danhmuc_id,
                 'from_date': nams,
                 }
            self.cr.execute(sql)
            duan_ids = [row[0] for row in self.cr.fetchall()]
            if duan_ids and giaidoan not in giaidoan_ids:
                giaidoan_ids.append(giaidoan)
        giaidoans = giaidoan_obj.browse(self.cr, self.uid, giaidoan_ids)
        return giaidoans
    
    def get_duan_line(self, danhmuc, giaidoan):#chua xu li
        duan_obj = self.pool.get('duan.dautu')
        wizard_data = self.localcontext['data']['form']
        from_date = wizard_data['date_from']
        to_date = wizard_data['date_to']
        ...
        for nam in nams:
            ...
            self.cr.execute(sql)
            duan_ids = [row[0] for row in self.cr.fetchall()]
            for duan in duan_obj.browse(self.cr, self.uid, duan_ids):
                if duan.id not in dsda:
                    dsda[duan.id] = {
                        'duan': duan,
                        'name': duan.name,
                        'nhom': duan.nhom_id and duan.nhom_id.name or False,
                        'diadiem': duan.quan_huyen_id and duan.quan_huyen_id.name or False,
                        'nanglucthietke': duan.nanglucthietke.name,
                        'tongmuc_dautu': duan.tongmuc_dautu,
                        'namkhoicong': duan.namkhoicong,
                        'namhoanthanh': duan.namhoanthanh,
                        'qd_soquyetdinh': self.get_qd_dautu(duan) and self.get_qd_dautu(duan)['qd_dautu_soquyetdinh'] or False,
                        'qd_ngaydautu': self.get_qd_dautu(duan) and self.get_qd_dautu(duan)['qd_dautu_ngay'] or False,
                        'tkkt_tdt_soquyetdinh': self.get_khoiluong_pheduyet_tkkt_tdt(duan) and self.get_khoiluong_pheduyet_tkkt_tdt(duan)['tkkt_tdt_soquyetdinh'] or False,
                        'tkkt_date': self.get_khoiluong_pheduyet_tkkt_tdt(duan) and self.get_khoiluong_pheduyet_tkkt_tdt(duan)['tkkt_tdt_ngaythangnam'] or '',
                        'tkkt_tongdutoan': self.get_khoiluong_pheduyet_tkkt_tdt(duan) and self.get_khoiluong_pheduyet_tkkt_tdt(duan)['tkkt_tdt_tongdutoan'] or 0,
                        'kqqd_soquyetdinh': self.get_qd_kqdt(duan) and self.get_qd_kqdt(duan)['kq_pheduyet_kqdt_soquyetdinh'] or False,
                        'kqqd_ngaydautu': self.get_qd_kqdt(duan) and self.get_qd_kqdt(duan)['kq_pheduyet_kqdt_ngay'] or False,
                        'kq_pheduyet_giagoithau': self.get_qd_kqdt(duan) and self.get_qd_kqdt(duan)['kq_pheduyet_giagoithau'] or 0,
                        'ghichu': duan.ghichu,
                        'chudautu': duan.nha_dau_tu_id and duan.nha_dau_tu_id.name or False,
                        'luyke_capphat_namtruoc': self.get_kehoachvon(duan) and self.get_kehoachvon(duan)['luyke_capphat_namtruoc'] or 0,
                        'kehoach_daduyet_gannhat': self.get_kehoachvon(duan) and self.get_kehoachvon(duan)['kehoach_daduyet_gannhat'] or 0,
                    }
                else:
                    dsda[duan.id].update({'tongmuc_dautu': duan.tongmuc_dautu+dsda[duan.id]['tongmuc_dautu']})
                    dsda[duan.id].update({'tkkt_tongdutoan': self.get_khoiluong_pheduyet_tkkt_tdt(duan) and self.get_khoiluong_pheduyet_tkkt_tdt(duan)['tkkt_tdt_tongdutoan'] or 0+dsda[duan.id]['tkkt_tongdutoan']})
                    dsda[duan.id].update({'kq_pheduyet_giagoithau': self.get_qd_kqdt(duan) and self.get_qd_kqdt(duan)['kq_pheduyet_giagoithau'] or 0+dsda[duan.id]['kq_pheduyet_giagoithau']})
                    dsda[duan.id].update({'luyke_capphat_namtruoc': self.get_kehoachvon(duan) and self.get_kehoachvon(duan)['luyke_capphat_namtruoc'] or 0+dsda[duan.id]['luyke_capphat_namtruoc']})
                    dsda[duan.id].update({'kehoach_daduyet_gannhat': self.get_kehoachvon(duan) and self.get_kehoachvon(duan)['kehoach_daduyet_gannhat'] or 0+dsda[duan.id]['kehoach_daduyet_gannhat']})
        for da in dsda.itervalues():
            duans.append(da)
        return duans
        ...
        if duan_object:
            for kq in duan_object.pheduyet_kqdt_lines:
                values ={
                     ...
                     'kq_pheduyet_giagoithau':kq.pheduyet_giatrungthau or False,
    ...
    def get_kehoachvon(self, duan_object):
        ...
        for nam in nams:
            ...
            luyke_capphat_namtruoc += kehoachvon and kehoachvon['luyke_capphat_namtruoc'] or 0
            kehoach_daduyet_gannhat += kehoachvon and kehoachvon['kehoach_daduyet_gannhat'] or 0
            uoc_thuchien += kehoachvon and kehoachvon['uoc_thuchien'] or 0
            chudautu_dexuat += kehoachvon and kehoachvon['chudautu_dexuat'] or 0
            kehoach_dieuhoa += kehoachvon and kehoachvon['kehoach_dieuhoa'] or 0
        vals = {
            'luyke_capphat_namtruoc': luyke_capphat_namtruoc,
            'kehoach_daduyet_gannhat': kehoach_daduyet_gannhat,
            'uoc_thuchien': uoc_thuchien,
            'chudautu_dexuat': chudautu_dexuat,
            'kehoach_dieuhoa': kehoach_dieuhoa,
