...
class Parser(report_sxw.rml_parse):
    ...
    def get_chitiet_congno(self, partner_id):
        ...
        sql = '''
            select ai.date_invoice as ngay,ai.name as maphieudexuat,rp.ma_doi_tuong as madoituong,rp.name as tendoituong,
                (ai.so_tien+ai.sotien_lai) as no, (ai.so_tien+ai.sotien_lai-ai.residual-ai.sotien_lai_conlai) as co
            
                from account_invoice ai
                left join res_partner rp on rp.id = ai.partner_id
                
                where ai.partner_id=%s and ai.state in ('open') and ai.date_invoice between '%s' and '%s' and ai.chinhanh_id=%s
                    and ai.mlg_type='%s'
    ...
    def get_tongcong(self, partner_id):
        ...
        mlg_type = wizard_data['mlg_type']
        sql = '''
            select case when sum(residual+sotien_lai_conlai)!=0 then sum(residual+sotien_lai_conlai) else 0 end tongtien from account_invoice
