...
class Parser(report_sxw.rml_parse):
        ...
        if partner_id:
            ...
            sql = '''
                select case when sum(so_tien_no)!=0 then sum(so_tien_no) else 0 end nodauky
                    from congno_dauky_line where mlg_type in ('no_doanh_thu',
                                                              'chi_ho_dien_thoai',
                                                              'phai_thu_bao_hiem',
                                                              'phai_thu_ky_quy',
                                                              'phat_vi_pham',
                                                              ...
                                                              'thu_phi_thuong_hieu',
                                                              'tra_gop_xe',
                                                              'hoan_tam_ung') and chinhanh_id=%s
            ...
            self.cr.execute(sql)
            return self.convert_amount(self.cr.fetchone()[0])
        return 0
    ...
    def get_chitiet_congno(self, partner_id):
        ...
        period = self.pool.get('account.period').browse(self.cr, self.uid, period_id[0])
        sql = '''
            select ai.date_invoice as ngay,ai.number as maphieudexuat,rp.ma_doi_tuong as madoituong,rp.name as tendoituong,
