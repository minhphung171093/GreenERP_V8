    ...
    def get_line(self):
        ...
        sql = '''
            select ai.name as ma_giaodich, ai.date_invoice as ngay_giaodich, cn.code as ma_chinhanh, cn.name as ten_chinhanh,
                ...
                ai.so_hop_dong as so_hop_dong, bsx.name as bien_so_xe, ai.so_hoa_don as so_hoa_don,lvp.name as loaivipham,
                ...
                left join loai_vi_pham lvp on ai.loai_vipham_id = lvp.id
                where ai.state='open' and ai.chinhanh_id=%s and ai.date_invoice between '%s' and '%s' and ai.mlg_type in ('phat_vi_pham') 
        '''%(chinhanh_id[0],from_date,to_date)

        loai_vipham = wizard_data['loai_vipham_id']
        if loai_vipham:
            sql+='''
