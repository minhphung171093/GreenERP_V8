...
class doanh_so_phaitra_wizard(osv.osv_memory):
    _name = "doanh.so.phaitra.wizard"
    ...
    def review_report(self, cr, uid, ids, context=None):
        ...
        for line in cr.dictfetchall():
            doanhsophaitra_line.append((0,0,{
        ...
        vals = {
            ...
            'doanhsophaitra_line':doanhsophaitra_line,
        }
        report_id = self.pool.get('doanh.so.phaitra').create(cr, uid, vals)
        res = self.pool.get('ir.model.data').get_object_reference(cr, uid, 
                                        'mlg_arap_account', 'view_doanh_so_phaitra_form')
        return {
                    'name': 'Doanh số phải trả',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'res_model': 'doanh.so.phaitra',
                    'domain': [],
    ...
    def print_report(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        datas = {'ids': ids}
        datas['model'] = 'doanh.so.phaitra.wizard'
        datas['form'] = self.read(cr, uid, ids)[0]
        datas['form'].update({'active_id':ids[0]})
        return {'type': 'ir.actions.report.xml', 'report_name': 'doanh_so_phaitra_wizard_report', 'datas': datas}
        
doanh_so_phaitra_wizard()

class doanh_so_phaitra(osv.osv_memory):
    _name = "doanh.so.phaitra"
    
    _columns = {
        'from_date': fields.date('Ngày bắt đầu'),
        'to_date': fields.date('Ngày kết thúc'),
        'doanhsophaitra_line': fields.one2many('doanh.so.phaitra.line', 'doanhsophaitra_id', 'Chi tiết'),
        'chinhanh_id': fields.many2one('account.account','Chi nhánh'),
    }
    ...
    def print_report(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        datas = {'ids': ids}
        datas['model'] = 'doanh.so.phaitra'
        datas['form'] = self.read(cr, uid, ids)[0]
        datas['form'].update({'active_id':ids[0]})
        return {'type': 'ir.actions.report.xml', 'report_name': 'doanh_so_phaitra_report', 'datas': datas}
    
doanh_so_phaitra()

class doanh_so_phaitra_line(osv.osv_memory):
    _name = "doanh.so.phaitra.line"
