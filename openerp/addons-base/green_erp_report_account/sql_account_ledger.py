...
class sql_account_ledger(osv.osv):
    ...
    def get_line(self, cr, start_date, end_date, account_id, showdetail, company_id):
        if showdetail:
            sql = '''
                SELECT gl_date, doc_date, doc_no, description, acc_code, 
                    nullif(amount_dr,0) debit, nullif(amount_cr,0) credit
                FROM fin_ledger_report('%s','%s', %s, %s, %s);
            ''' %(start_date, end_date, account_id, showdetail, company_id)
        else:
            sql = '''
                SELECT gl_date, doc_date, doc_no, description, acc_code, 
...
#             return True
        sql = '''
        DROP FUNCTION IF EXISTS fin_ledger_report(date, date, integer, boolean, integer) CASCADE;
        commit;
        CREATE OR REPLACE FUNCTION fin_ledger_report(date, date, integer, boolean, integer)
        ...
        $BODY$
          ...
          COST 100
          ROWS 1000;
        ALTER FUNCTION fin_ledger_report(date, date, integer, boolean, integer)
          OWNER TO openerp;
        '''
        cr.execute(sql)
        return True
