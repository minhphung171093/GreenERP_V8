...
class sql_general_ledger(osv.osv):
    ...
    def get_line(self, cr, start_date, end_date, company_id):
        ...
        FROM fin_general_ledger_report('%s', '%s', %s)
...
#             return True
        sql = '''
        DROP FUNCTION IF EXISTS fin_general_ledger_report(date, date, integer) CASCADE;
        commit;
        CREATE OR REPLACE FUNCTION fin_general_ledger_report(date, date, integer)
        ...
        END; $BODY$
          ...
          COST 100
          ROWS 1000;
        ALTER FUNCTION fin_general_ledger_report(date, date, integer)
          OWNER TO openerp;
        '''
        cr.execute(sql)
        return True
